...
class RealImageGenerationDemo:
    ...
    def __init__(self):
        ...
        # Initialize real services with proper parameters
        redis_url = os.getenv("REDIS_URL", "redis://localhost:6379")
        self.redis_store = RedisStore(redis_url=redis_url, max_size=100)

        # VectorStore can use defaults or environment variables
        vector_db_type = os.getenv("VECTOR_DB_TYPE", "chroma")
        self.vector_store = VectorStore(
            persist_directory="./chroma_db", vector_db_type=vector_db_type
        )

        self.prompt_builder = PromptBuilder(self.redis_store, self.vector_store)
