...
export async function GET(request: NextRequest) {
  try {
    ...
    if (reflectionsError) {
      ...
      return NextResponse.json({ error: reflectionsError.message, code: reflectionsError.code }, { status: 500 });
    }
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (typeof mood !== 'string' || typeof note !== 'string' || !Array.isArray(tags)) {
      return NextResponse.json({ error: 'Invalid data types for reflection fields' }, { status: 400 });
    }
...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (mood !== undefined && typeof mood !== 'string') {
      return NextResponse.json({ error: 'Invalid data type for mood' }, { status: 400 });
    }
    if (note !== undefined && typeof note !== 'string') {
      return NextResponse.json({ error: 'Invalid data type for note' }, { status: 400 });
    }
    if (tags !== undefined && !Array.isArray(tags)) {
      return NextResponse.json({ error: 'Invalid data type for tags' }, { status: 400 });
    }
    ...
    if (updateError) {
      ...
      if (updateError.code === 'PGRST116') { // PGRST116: "The result contains 0 rows"
        return NextResponse.json({ error: 'Reflection not found or user does not have permission to update' }, { status: 404 });
      }
